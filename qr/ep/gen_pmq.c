...
void fillb(int npoints, int rank, double npol, double points[], double pol[], double b[]);
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 6) {
        printf("Uso: ./gen_pmq [qtde de linhas LI em A] [grau do polinomio gerador dos pontos] [quantidade de pontos gerados (n)] [grau do polinomio para solucionar o problema (m)] [semente do gerador]\n");
        return 0;
    }
    rank = atoi(argv[1]);
    npol = atoi(argv[2]);
    ...
    fillb(npoints, rank, npol, points, pol, b);
    disturb(npoints, b);
...
void genRandPol(int npol, double pol[]) {
    int i;
    for (i = 0; i <= npol; i++) {
        pol[i] = rand() / (double) (RAND_MAX / 1.0);
...
void genPoints(int npoints, double points[]) {
    int i;
    for (i = 0; i < npoints; i++) {
        points[i] = rand()  / (double) (RAND_MAX / 1.0);
        if (rand() > RAND_MAX / 2.0) 
            points[i] *= -1;
    }
}
